...
public class CameraController : MonoBehaviour
{
    ...
    void LateUpdate()
    {
        ...
        float targetX = ((p1.position.x + p2.position.x) * 0.5f) + ((v1.x + v2.x) * 0.5f * lookAhead);
        float targetY = ((p1.position.y + p2.position.y) * 0.5f) + ((v1.y + v2.y) * 0.5f * lookAhead);

        transform.position = Vector3.Lerp(transform.position, new Vector3(targetX, targetY, -10f), smoothing * Time.deltaTime);
        ...
        _cam.orthographicSize = Mathf.Lerp(_cam.orthographicSize, targetSize < minSize ? minSize : targetSize, smoothing * Time.deltaTime);
    }
}
