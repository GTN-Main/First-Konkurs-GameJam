...
public class PlayersPosManager : MonoBehaviour
{
    ...
    {
        if (
            PlayerSpawnManager.Instance.Player1 == null
            || PlayerSpawnManager.Instance.Player2 == null
        )
            return this.gameObject; // Return self if any of the players is missing
        float sqrDistanceToPlayer1 = Vector2.SqrMagnitude(
            PlayerSpawnManager.Instance.Player1.transform.position - (Vector3)point
        );
        float sqrDistanceToPlayer2 = Vector2.SqrMagnitude(
            PlayerSpawnManager.Instance.Player2.transform.position - (Vector3)point
        );
